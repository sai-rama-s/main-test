...
def RunCoRegV2(PARAMS):
    ...
    ###############################################################################################################
    # Update Output Dir
    PARAMS["outputs"]["output_path"] = PARAMS["outputs"]["output_path"] + os.path.splitext(os.path.basename(PARAMS["inputs"]["optical_path"]))[0] + "_" + os.path.splitext(os.path.basename(PARAMS["inputs"]["sar_path"]))[0] + "/"
    ...
    DEFAULT_OPTIONS_COMMON["path"] = PARAMS["outputs"]["output_path"] + DEFAULT_OPTIONS_COMMON["dir"] + DEFAULT_OPTIONS_COMMON["path"]
    DEFAULT_OPTIONS_OPTICAL["path"] = PARAMS["outputs"]["output_path"] + DEFAULT_OPTIONS_OPTICAL["dir"] + DEFAULT_OPTIONS_OPTICAL["path"]
    DEFAULT_OPTIONS_SAR["path"] = PARAMS["outputs"]["output_path"] + DEFAULT_OPTIONS_SAR["dir"] + DEFAULT_OPTIONS_SAR["path"]
    ...
    if options_common["save"] or options_optical["save"] or options_sar["save"]:
        ...
        RefreshOutputDirectories(PARAMS["outputs"]["output_path"], SubDirs)
    ###############################################################################################################
    # SETUP #######################################################################################################
    ###############################################################################################################
    ...
    detectParams_optical = {
        "thresholds": {
            "
            _threshold": PARAMS["params"]["keypoint_detection"]["optical"]["thresholds"]["log_threshold"]
        },
        "windows": {
    ...
    if RunSteps[3]:
        ...
        if MatchMethod == "Template":
            I_matched, matchIndices, matchDistances = ImageRegister_TemplateMatch(keypoints_1, keypoints_2, match_descriptors_1, match_descriptors_2, 
    ...
    # Final Outputs
    I_1_bands = None
    ext_1 = os.path.splitext(optical_path)[1]
    if ext_1 in [".tif", ".tiff"]:
        clipped_r =  rasterio.open(optical_path).read(1)
        clipped_g =  rasterio.open(optical_path).read(2)
        clipped_b =  rasterio.open(optical_path).read(3)
        rgb_list = [clipped_r, clipped_g, clipped_b]
    rgb_norm_list = []
    ...
    #plt.imsave(options["path"], I_1_bands_norm, dpi=10000)
    plt.figure(figsize=(15, 40))
    plt.axis('off')
    plt.imshow(I_rgb_norm)
    plt.savefig(options["path"], format="jpeg", bbox_inches='tight')
    plt.show()
    
    # cv2.imwrite(options["path"], I_1_bands_norm)
    options["path"] = options_common["path"].format("SAR_Final")
    fig, ax = plt.subplots(1, figsize=(15, 40))
    plt.imshow(I_2_gray_corrected, cmap = 'gray', vmax = np.max(I_2_gray_corrected) // 16 , vmin = 20)
    plt.axis('off')
    plt.savefig(options["path"], format="jpeg",  bbox_inches='tight')
    plt.show()
    #plt.imsave(options["path"], I_2_bands_norm_corrected, dpi=10000, cmap='gray')
    ...
    #plt.axis('off')
    #plt.imshow(I_rgb_norm)
    #plt.savefig('optic.jpg', format="jpeg", bbox_inches='tight')
    #plt.show()
   ...
   # s3_client.upload_file('./optical_v2.jpg', 'sentinel-api-test', 'optic_v2.jpg')
    #s3_client.upload_file('./sar_v2.jpg', 'sentinel-api-test', 'sar_v2.jpg')
    

    if PARAMS["outputs"]["s3"]:
        # Connect to S3
        s3_client = boto3.client('s3',
            aws_access_key_id='AKIAQVSA7EXYD5OVVQMD',
            aws_secret_access_key='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'
        ...
        # Upload to S3
        s3_client.upload_file(options_common["path"].format("Optical_Final"), PARAMS["outputs"]["s3_bucket"], PARAMS["outputs"]["s3_path"] + "Optical.jpg")
        s3_client.upload_file(options_common["path"].format("SAR_Final"), PARAMS["outputs"]["s3_bucket"], PARAMS["outputs"]["s3_path"] + "Sar.jpg")
...
def RunCoRegV2(PARAMS):
   ...
   # with rasterio.open(PARAMS["outputs"]["order_id"]+'Sar.tif', 'w', **sar_meta) as dst:
    ...
    Source= 'kush.thakker@galaxeye.space',
    Destination= {
      'ToAddresses': ['kushthakker@aol.com'],
    },
     Message={
        "Subject": {
            "Charset": 'UTF-8',
             "Data": 'Your order has been created successfully',
      },
        'Body': {
            'Text': {
                'Data': 'string',
            ...
            'Html': {
                'Data': f'Your data with {os.environ["ORDER_ID"]} has been processed and is ready for download. Please click on the link below to download the data. <a href="http://localhost:4000/order/{os.environ["ORDER_ID"]}">Download</a>',
                'Charset': 'string'
            }
        }
    },
...
query_handler(os.environ['ORDER_ID'], "Processed")
