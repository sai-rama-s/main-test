...
class LogGabor(Image):
    ...
    def band(self, sf_0, B_sf, force=False):
        ...
        if sf_0 == 0.:
            ...
            f_fixed = np.copy(self.f)
            f_min_fixed = np.min(f_fixed[f_fixed != 0.0])
            f_fixed[f_fixed == 0.0] = f_min_fixed
            # print(np.min(f_fixed), np.max(f_fixed))
            env = (1.0/f_fixed) * (np.exp(-0.5*(np.log(f_fixed/sf_0)**2)/B_sf**2))
        return env
